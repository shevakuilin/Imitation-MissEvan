...
- (void)setSelectIndex:(NSInteger)selectIndex animation:(BOOL)animation
{
    ...
    if (selectIndex < self.items.count) {
        ...
        //显得长度，可能会动态改变
        self.lineLayer.frame = CGRectMake(25, CGRectGetHeight(item.frame) - self.lineHeight, 35, 10);//CGRectMake(CGRectGetMinX(item.frame), CGRectGetHeight(item.frame) - self.lineHeight, 60, self.lineHeight);
